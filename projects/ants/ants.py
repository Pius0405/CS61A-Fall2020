...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        location = 0
        while current_place != beehive and location <= self.max_range:
            bee =  rANTdom_else_none(current_place.bees)
            if bee != None:
                if self.min_range <= location and location<= self.max_range:
                    return bee
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        """
        # BEGIN Problem EC
        if self is not QueenAnt.trueQueen:
            Insect.reduce_armor(self,self.armor)
        else:
            current_place = self.place.exit
            while current_place != None:
                if current_place.ant not in QueenAnt.doubled and current_place.ant != None:
...
class Bee(Insect):
    ...
    # OVERRIDE CLASS ATTRIBUTES HERE
    is_watersafe = True
    def __init__(self,armor,direction = "left"):
        Insect.__init__(self,armor)
    ...
    def blocked(self):
        ...
        return not (not self.place.ant or not self.place.ant.blocks_path)
...
def apply_status(status, bee, length):
    ...
    # BEGIN Problem Optional 4
    original_method = bee.action
    modified_method = status(original_method, bee)
    def new_action(gamestate):
        nonlocal length
        if length > 0:
            modified_method(gamestate)
            length -= 1
            if length == 0:
                bee.direction = "left"
        else:
            original_method(gamestate)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        while current_place != beehive:
            if current_place.ant is not self and current_place.ant:
                insect_dict.update({current_place.ant : distance})
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 5
        damage = LaserAnt.damage - (self.insects_shot*0.05) - (distance * 0.2)
        if damage <= 0:
            return 0
        else:
            return damage
