...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def check_word(para):
        word_list = split(lower(remove_punctuation(para)))
        for word in topic:
...
def time_per_word(times_per_player, words):
    ...
    """
    # BEGIN PROBLEM 9
    time = [[rec[i+1] - rec[i] for i in range(len(rec)) if i < len(rec)-1] for rec in times_per_player]
...
def fastest_words(game):
    ...
    """
    time = all_times(game)
    player_indices = range(len(time))  # contains an *index* for each player
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if time[j][i] < fastest:
                fastest = time[j][i]
                fastest_index = j
        result[fastest_index].append(words[i])
    return result            
    # END PROBLEM 10
