...
def roll_dice(num_rolls, dice=six_sided):
    ...
    assert num_rolls <= 10
    score = 0
    flag = False #True if there is 1 outcome else False
...
def swine_align(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4a
    #GDC is always smaller than or equal to the smallest number in the sequence
    #smallest possible GDC is 1
    if player_score == 0 or opponent_score == 0:
        return False
    else:
        GDC = min(player_score, opponent_score)
        while True:
...
def pig_pass(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4b
    return opponent_score>player_score and opponent_score-player_score < 3
    # END PROBLEM 4b
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score_0, score_1):
        ...
        if who == 0:
            gain = score_0 - last_score
        else:
            gain = score_1 - last_score
        if gain > current_high:
            print(gain,"point(s)! The most yet for Player",who)
            current_high = gain
        return announce_highest(who, score_0 if who == 0 else score_1, current_high)
    return say
    # END PROBLEM 7
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    bacon_strategy_result = bacon_strategy(score, opponent_score, cutoff, num_rolls)
    if extra_turn(score+free_bacon(opponent_score),opponent_score) or bacon_strategy_result == 0:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if difference > 10 and score > 80:
        rolls = extra_turn_strategy(score, opponent_score, cutoff = 7, num_rolls = 5)
    elif difference > 10 and score <= 80:
        rolls = extra_turn_strategy(score, opponent_score, cutoff = 7, num_rolls = 5)
    elif difference < 10 and score > 80:
        rolls = extra_turn_strategy(score, opponent_score, cutoff = 7, num_rolls = 5)
    else:
        rolls = extra_turn_strategy(score, opponent_score, cutoff = 9, num_rolls = 9)
    return rolls
    # END PROBLEM 12
