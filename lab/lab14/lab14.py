...
def prune_min(t):
    ...
    """
    if not t.is_leaf():
        first_branch = t.branches[0]
        second_branch = t.branches[1]
        if first_branch.label > second_branch.label:
            t.branches.remove(first_branch)
        else:
            t.branches.remove(second_branch)
...
def num_splits(s, d):
    ...
    """
    """Method of solving:
    1. Construct two different lists (2 sublists) by placing 
    an element in each list starting from the first element.
    2. Once the last element has been reached, compute the abs difference between the two sublist
    3. Return the result divided by two since two duplicated results are calculated for each possible subset
    """
    def helper(index, l1, l2):
        if index == len(s):
            if abs(sum(l1) - sum(l2)) <= d:
                return 1
            else:
                return 0
        else:
            in_first = helper(index + 1, l1+[s[index]], l2)
            in_second = helper(index + 1, l1, l2+[s[index]])
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            edit, cost = helper_align(skeleton_idx+1, code_idx+1)
            edit = skel_char + edit
            possibilities.append((edit, cost))
        # Insert
        edit, cost = helper_align(skeleton_idx, code_idx + 1)
        edit, cost = f"+[{code_char}]" + edit, cost + 1
        possibilities.append((edit, cost))
        # Delete
        edit, cost = helper_align(skeleton_idx + 1, code_idx)
        edit, cost = f"-[{skel_char}]" + edit, cost + 1
        possibilities.append((edit, cost))
...
def foldl(link, fn, z):
    ...
    if link is Link.empty:
        return z
    temp = fn(z, link.first) 
    return foldl(link.rest, fn, temp)
...
def filterl(lst, pred):
    ...
    """
    #using foldl
    def fn(current_list, val):
        if pred(val):
            return Link(val,current_list)
        else:
            return current_list
